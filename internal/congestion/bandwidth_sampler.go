// Adapted from tdragoun/quic-go bbr_v1 commit
// a07eb48492755adb24d4f278a92f5e054f1eccad (MIT), itself based on Google
// QUICHE BBRv1. Ported to the maintained fork's packet and monotime APIs.
package congestion
...
func (s *BandwidthSampler) OnPacketAcked(ackTime monotime.Time, lastAckedPacket protocol.PacketNumber) BandwidthSample {
	sentPacketState, ok := s.connectionStats.Get(lastAckedPacket)
	if !ok {
