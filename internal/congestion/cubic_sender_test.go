...
func TestCubicAvailablePacingBudgetIsCappedByPacerAndCWND(t *testing.T) {
	sender := newTestCubicSender(false)
	...
	sender.sender.pacer.SentPacket(now, 9*maxPacket)
	require.Equal(t, maxPacket, sender.sender.AvailablePacingBudget(now, 0), "pacing tokens must cap the congestion-window allowance")
}
